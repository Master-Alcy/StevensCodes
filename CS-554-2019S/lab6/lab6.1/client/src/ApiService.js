...
class ApiService {
  ...
  async getGraphQlData(resource, params, fields) {
      ...
      if (res.ok) {
        ...
        console.log("query",res.json());
          throw new Error(res.status);
      }
  }

  async mutationGraphQlData(resource, params, fields) {
      const command = `{${resource} ${this.paramsToString(params)} ${fields}}`
      const res = await fetch(this.apiUrl, {
          method: 'POST',
          mode: 'cors',
          headers: new Headers({
          ...
          body: JSON.stringify({query:`mutation${command}`}),
      });
      if (res.ok) {
          const body = await res.json();
          return body.data;
      } else {
          console.log("mutation",res.json);
          throw new Error(res.status);
      }
  }
  ...
  async createTodo(param) {
      const data = await this.mutationGraphQlData('createTodo', param, this.todoFields);
      return data;
  }
  async updataTodo(param) {
      const data = await this.mutationGraphQlData('updateTodo', param, this.todoFields);
      return data;
  }
  async deleteTodo(param) {
      const data = await this.mutationGraphQlData('deleteTodo', param, this.todoFields);
      return data;
  }
  /**
   *
