...
//turn cookie into object
const cookieParser = require("cookie-parser");
const movieData = require("./data.js");
// (This should be server side js)
...
//Middlewares has access to the request and response objects:
/** ◦Execute any code.
◦Make changes to the req/res objects.
◦End the request-response cycle.
◦Call the next middleware function in the stack 
Logging requests/Authentication/Access control/Caching data/Serialization

>>> Request is the request object, just like how we have access 
to the request in our routes.
>>> Response is the response object, just like how we have access 
to the response in our routes.
>>> next is a callback that will call the next middleware 
registered, or proceed to routes if none exist.
>>> If we do not call next(), we need to make sure we send a 
response of some sort or it will poll forever!

◦Register your middleware, optionally providing a path 
to apply that middleware to
...
// 2. One which will count the number of requests that 
// have been made to the current path
const pathsAccessed = {};
app.use(function(request, response, next) {
...
// 3. One which will log the last time the user has made a request, 
// and store it in a cookie.
//response.cookies(name, value, options) function/request.cookies object
app.use(function(request, response, next) {
  // If we had a user system, we could check to see 
  // if we could access /admin
  ...
  if (currentNumberOfRequests % 5 === 0) {
    ...
    // invalidate, then clear so that lastAccessed no longer 
    // shows up on the cookie object
    // response.cookie("lastAccessed", "", { expires: anHourAgo });
    response.clearCookie("lastAccessed");

    next();
    return; //return to prevent following sessions from running
  }
  ...
  // Providing a third parameter is optional, but allows you to 
...
/*
Middlewares can apply to entire app or a portion by
suplying a path as the first param to mid function
*/
...
app.use(function(request, response, next) {
  //in middleware 1 defined currentNo.Req
...
// 6. Where we check if there's an odd or e  ven request being made
app.use(function(request, response, next) {
