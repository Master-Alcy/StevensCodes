...
let rec extend_env_list en ids evs =
  match ids, evs with
  | [],[] -> en
  | id::ids, ev::evs -> extend_env_list (extend_env en id ev) ids evs
  | _ -> failwith "Number of arguments should match each other"

let rec apply_proc f a =
  match f with
  | ProcVal (ids,body,env) -> let evs = tupleVal_to_evs a in
    eval_expr (extend_env_list env ids evs) body
...
and
  eval_prog (AProg e) = eval_expr (empty_env ()) e
